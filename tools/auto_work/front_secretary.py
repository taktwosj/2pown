"""Compatibility wrapper for the extracted 1other front_secretary canonical.

This 1POW path is no longer the canonical implementation. It forwards execution
to `C:\\1other\\openclaw-front-secretary\\tools\\auto_work\\front_secretary.py`
so state/runtime do not diverge between the two roots.
"""
...
def canonical_script() -> Path:
    ...
    return root_other / "tools" / "auto_work" / "front_secretary.py"
...
def main() -> int:
    ...
    if not target.exists():
        print(
            "front_secretary canonical not found.\n"
            f"expected: {target}\n"
